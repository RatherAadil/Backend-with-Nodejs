...
// 2.Extract Auth Code and Exchange for ID Token
app.get('/auth/google/callback', async (req, res) => {
  const { sid } = req.cookies;
  const existingSession = sessions.find(({ sessionId }) => sessionId === sid);
  if (existingSession) {
    res.redirect(
      `http://localhost:5500/client/callback.html?sid=${existingSession.sessionId}`,
    );
    return res.end();
  }
  ...
  if (code) {
    ...
    if (existingUser) {
      ...
      res.redirect(
        `http://localhost:5500/client/callback.html?sid=${sessionId}`,
      );
      return res.end();
    }
    users.push(newUser);
