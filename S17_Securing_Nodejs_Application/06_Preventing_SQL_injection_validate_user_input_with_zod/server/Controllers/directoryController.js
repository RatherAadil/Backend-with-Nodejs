...
import { directoryNameSchema } from '../validators/directorySchema.js';
...
export const createDirectory = async (req, res, next) => {
  ...
  const { data: dirname, error } = validateInput(
    directoryNameSchema,
    req.headers.dirname,
  );
...
export const renameDirectory = async (req, res, next) => {
  const user = req.user;
  const { id } = req.params;
  const { data, error } = validateInput(directoryNameSchema, req.body);
  if (error) return res.status(400).json({ error });
