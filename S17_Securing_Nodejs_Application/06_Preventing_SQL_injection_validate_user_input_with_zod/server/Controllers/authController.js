...
import { otpSchema, verifyOTPSchema } from '../validators/authSchema.js';

export const sendLoginWithEmailOTP = async (req, res, next) => {
  const { data, error } = validateInput(otpSchema, req.body);
  if (error) {
...
export const verifyLoginWithEmailOTP = async (req, res, next) => {
  const { data, error } = validateInput(verifyOTPSchema, req.body);
  if (error) {
    return res.status(400).json({ error });
  }
