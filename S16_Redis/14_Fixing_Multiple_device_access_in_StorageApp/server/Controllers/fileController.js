...
import { VALID_RESOURCE_PERMISSIONS } from '../config/roles.js';
...
export const generateFileSharingLink = async (req, res, next) => {
  ...
  if (!enabled)
    return res.status(404).json({ error: 'This file is not accessible.' });
  try {
    const fileData = await File.findOne({
      _id: fileId,
      userId: user.id,
    ...
    res.status(200).json({
      ...
      success: true,
      data: {
        url: `http://localhost:4000/guest/access/${fileId}?token=${newToken}`,
...
export const getSharedFileInformation = async (req, res) => {
  ...
  const resData = {
    id,
    url: `http://localhost:4000/guest/file/view/${id}?token=${token}`,
    name: fileData.name,
